function OnMomiji01Spawn(keys)
    ...
    for i = 1, count do
        local unit = CreateUnitByName("ability_momiji_Spawn_unit",
                                      caster:GetOrigin() +
        ...
        unit:AddAbility("ability_thdots_momijiex")
        local ability = unit:FindAbilityByName("ability_thdots_momijiex")
        ability:SetLevel(1)
        ...
        unit:SetContextThink("ability_momiji01_Spawn_unit_regen", function()
            ...
            if GetDistanceBetweenTwoVec2D(caster:GetOrigin(), unit:GetOrigin()) <=
                keys.AuraDistance then
                unit:Heal(keys.RegenAmount / 5, caster)
                local abilityMomiji04Caster =
                    caster:FindAbilityByName("ability_thdots_momiji04")
                if abilityMomiji04Caster ~= nil then
                    local abilityMomiji04 =
                        unit:FindAbilityByName("ability_thdots_momiji04")
                    if abilityMomiji04 == nil then
                        unit:AddAbility("ability_thdots_momiji04")
                        abilityMomiji04 =
                            unit:FindAbilityByName("ability_thdots_momiji04")
                        abilityMomiji04:SetLevel(
                            abilityMomiji04Caster:GetLevel())
                    elseif abilityMomiji04:GetLevel() ~=
                        abilityMomiji04Caster:GetLevel() then
                        abilityMomiji04:SetLevel(
                            abilityMomiji04Caster:GetLevel())
                    end
                end
            else
                local abilityMomiji04 = unit:FindAbilityByName(
...
function OnMomiji02Start(keys)
    ...
    AddFOWViewer(caster:GetTeam(), keys.target_points[1], keys.Radius, Duration,
                 false)
    ...
    local unit = CreateUnitByName("npc_vision_momiji_dummy_unit",
