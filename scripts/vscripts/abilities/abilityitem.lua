...
function ForceStaff(keys)
    ...
    ability.forced_distance = ability:GetLevelSpecialValueFor("push_length",
                                                              ability_level)
    ability.forced_speed = ability:GetLevelSpecialValueFor("push_speed",
                                                           ability_level) * 1 /
                               30 -- * 1/30 gives a duration of ~0.4 second push time (which is what the gamepedia-site says it should be)
...
function ForceHorizontal(keys)
    ...
    if ability.forced_traveled < ability.forced_distance then
        target:SetAbsOrigin(target:GetAbsOrigin() + ability.forced_direction *
                                ability.forced_speed)
        ability.forced_traveled = ability.forced_traveled +
                                      (ability.forced_direction *
                                          ability.forced_speed):Length2D()
    else
...
function PrintTargetInfo(target)
    ...
    for i = 0, target:GetAbilityCount() - 1 do
        ...
        if Ability then
            print(
                indent .. "Target's Ability index:" .. tostring(i) .. " name:" ..
                    Ability:GetName())
            indent = indent .. "\t"
    ...
    for i = 0, target:GetModifierCount() - 1 do
        ...
        print(indent .. "Target's Modifier index:" .. tostring(i) .. " name:" ..
                  ModifierName)
        ...
        if ModifierClass then
            ...
            print(indent .. "GetStackCount:" ..
                      tostring(ModifierClass:GetStackCount()))
            print(indent .. "GetDuration:" ..
                      tostring(ModifierClass:GetDuration()))
            print(indent .. "GetRemainingTime:" ..
                      tostring(ModifierClass:GetRemainingTime()))
            print(indent .. "GetClass:" .. ModifierClass:GetClass())
            print(indent .. "GetCaster Name" ..
                      ModifierClass:GetCaster():GetName())
            print(indent .. "GetParent Name" ..
                      ModifierClass:GetParent():GetName())
        end
    end
end
...
function ItemAbility_Debug_GetGold(keys)
    ...
    PlayerResource:SetGold(CasterPlayerID, PlayerResource:GetUnreliableGold(
                               CasterPlayerID) + keys.bonus_gold, false)
end
function ItemAbility_Debug_PrintHealth(keys)
    if (keys.caster) then
        DebugPrint(keys.debug_string .. "caster health is " ..
                       keys.caster:GetHealth())
    end
    if (keys.target) then
        DebugPrint(keys.debug_string .. "target health is " ..
                       keys.target:GetHealth())
    end
end

function DebugPrint(msg) if ITEM_DEBUG == true then print(msg) end end

function DebugFunction(func, ...) if ITEM_DEBUG then func(...) end end
---------------------------------------------------------------------------------------
-- UTIL Functions
...
function PrintKeys(keys) PrintTable(keys) end
...
function ItemAbility_ModifierTarget(keys)
    ...
    if Caster and Target and Caster:GetTeam() ~= Target:GetTeam() then
        if keys.Blockable == 1 and is_spell_blocked_by_hakurei_amulet(Target) then
            return
        elseif (not keys.ApplyToTower or keys.ApplyToTower == 0) and
            Target:IsBuilding() then
            return
        end
    end
...
function ItemAbility_SetModifierStackCount(keys)
    ...
    if (keys.ModifierCount > 0) then
        if (Target:HasModifier(keys.ModifierName) == false) then
            ...
            ItemAbility:ApplyDataDrivenModifier(Caster, Target,
                                                keys.ModifierName, Params)
        end
        Target:SetModifierStackCount(keys.ModifierName, ItemAbility,
                                     keys.ModifierCount)
...
function ItemAbility_ModifyModifierStackCount(keys)
    ...
    if (Target:HasModifier(keys.ModifierName)) then
        ModifierStackCount = Target:GetModifierStackCount(keys.ModifierName,
                                                          Caster)
    end
...
function create_illusion(keys, illusion_origin, illusion_incoming_damage,
                         illusion_outgoing_damage, illusion_duration)
    local player_id = keys.caster:GetPlayerID()
    local caster_team = keys.caster:GetTeam()

    local illusion = CreateUnitByName(keys.caster:GetUnitName(),
                                      illusion_origin, true, keys.caster,
                                      keys.caster, caster_team) -- handle_UnitOwner needs to be nil, or else it will crash the game.
    ...
    for i = 1, caster_level - 1 do illusion:HeroLevelUp(false) end
    ...
    for ability_slot = 0, 15 do
        ...
        if individual_ability ~= nil then
            local illusion_ability = illusion:FindAbilityByName(
                                         individual_ability:GetAbilityName())
    ...
    for item_slot = 0, 5 do
        ...
        if individual_item ~= nil then
            local illusion_duplicate_item =
                CreateItem(individual_item:GetName(), illusion, illusion)
...
function ItemAbility_CardWorseMan_OnSpellStart(keys)
    ...
    SendOverheadEventMessage(nil, OVERHEAD_ALERT_MANA_LOSS, Target, AbsorbMana,
                             nil)
    ...
    SendOverheadEventMessage(nil, OVERHEAD_ALERT_BONUS_SPELL_DAMAGE, Target,
                             keys.AbsorbDamage, nil)
    --	end
    ItemAbility_SpendItem(keys)
end
function ItemAbility_CardLoveMan_OnSpellStart(keys)
    ...
    SendOverheadEventMessage(nil, OVERHEAD_ALERT_HEAL, Target, keys.HealAmount,
                             nil)
    ItemAbility_SpendItem(keys)
end
function ItemAbility_MrYang_OnSpellStart(keys)
    ...
    if is_spell_blocked_by_hakurei_amulet(Target) then return end

    ItemAbility:ApplyDataDrivenModifier(Caster, Target,
                                        keys.DeclineIntDebuffName, {})

    for i = 0.0, keys.SlowdownDuration, keys.SlowdownInterval do
        ItemAbility:ApplyDataDrivenModifier(Caster, Target,
                                            keys.SlowdownDebuffName,
                                            {duration = i})
    end
end
...
function ItemAbility_SmashStick_OnAttack(keys)
    ...
    if (Caster:IsRealHero() and Target:IsBuilding() == false) then
        if Caster:GetAttackCapability() == DOTA_UNIT_CAP_RANGED_ATTACK then
            DebugPrint("ItemAbility_SmashStick_OnAttack| Ranged Stun: " ..
                           keys.StunDurationRanged .. " sec")
            UnitStunTarget(Caster, Target, keys.StunDurationRanged)
        elseif Caster:GetAttackCapability() == DOTA_UNIT_CAP_MELEE_ATTACK then
            DebugPrint("ItemAbility_SmashStick_OnAttack| Melee Stun: " ..
                           keys.StunDurationMelee .. " sec")
...
function ItemAbility_WindGun_OnAttack(keys)
    ...
    if (Caster:IsRealHero() and Target:IsRealHero() == false and
        Target:IsBuilding() == false) then
        local damage_table = {
            ability = ItemAbility,
            victim = Target,
            attacker = Caster,
...
function ItemAbility_Camera_OnAttack(keys)
    ...
    local Target = keys.target
    if (Caster:IsRealHero() and Target:IsBuilding() == false) then
        local damage_to_deal =
            Target:GetMaxHealth() * keys.DamageHealthPercent * 0.01
        if Target:GetUnitName() == "npc_dota_roshan" then
            damage_to_deal = damage_to_deal * 0.1
        end
        local damage_table = {
            ability = ItemAbility,
            victim = Target,
...
function ItemAbility_Verity_OnAttack(keys)
    ...
    if (Target:IsBuilding() == false and Caster:IsIllusion() == false) then
        ...
        local RemoveMana =
            Target:GetMaxMana() * keys.PenetrateRemoveManaPercent * 0.01
...
function ItemAbility_Kafziel_OnAttack(keys)
    ...
    local Target = keys.target
    if (Caster:IsRealHero() and Target:IsBuilding() == false) then
        local damage_to_deal = (Caster:GetHealth() - Target:GetHealth()) *
                                   keys.HarvestDamageFactor
        if (damage_to_deal > 0) then
            local damage_table = {
                ability = ItemAbility,
                victim = Target,
            ...
            UnitDamageTarget(damage_table)
            SendOverheadEventMessage(nil, OVERHEAD_ALERT_BONUS_SPELL_DAMAGE,
                                     Target, damage_to_deal, nil)
...
function ItemAbility_Frock_Poison(keys)
    ...
    if (Attacker:IsBuilding() == false) then
        ...
        if (Attacker:IsHero()) then
            local MaxAttribute = max(max(Attacker:GetStrength(),
                                         Attacker:GetAgility()),
                                     Attacker:GetIntellect())
            damage_to_deal = keys.PoisonDamageBase + MaxAttribute *
                                 keys.PoisonDamageFactor
        end
        ...
        if (damage_to_deal > 0) then
            ...
            UnitDamageTarget(damage_table)
            SendOverheadEventMessage(nil, OVERHEAD_ALERT_BONUS_POISON_DAMAGE,
                                     Attacker, damage_to_deal, nil)
...
function ItemAbility_Frock_Poison_TakeDamage(keys)
    ...
    local Caster = keys.caster
    local Attacker = keys.attacker
    local damage_to_deal = keys.TakenDamage * 0.75
    if Caster:GetContext("ability_yuyuko_Ex_deadflag") ~= nil and
        keys.TakenDamage == Caster:GetMaxHealth() then damage_to_deal = 0 end
    if (Attacker:IsBuilding() == false) and Attacker ~= Caster and
        Attacker:HasModifier("modifier_item_frock_OnTakeDamage") == false then
...
function ItemAbility_LoneLiness_RegenHealth(keys)
    ...
    local HealAmount =
        ((MaxHealth - Health) / 2) * keys.PercentHealthRegenBonus * 0.01 +
            HealthRegen * keys.HealthRegenMultiplier * 0.01
...
function ItemAbility_HakureiTicket_FeastHeal(keys)
    ...
    for i, v in pairs(Targets) do
        ...
        SendOverheadEventMessage(nil, OVERHEAD_ALERT_HEAL, v,
                                 HealAmount + v:GetMaxHealth() * 0.1, nil)
        local effectIndex = ParticleManager:CreateParticle(
                                "particles/items2_fx/mekanism.vpcf",
                                PATTACH_CUSTOMORIGIN, v)
        ParticleManager:SetParticleControlEnt(effectIndex, 0, v, 5,
                                              "follow_origin", Vector(0, 0, 0),
                                              true)
    end
end
...
function ItemAbility_DummyDoll_OnSpellStart(keys)
    ...
    illusion = create_illusion(keys, Caster:GetAbsOrigin(),
                               keys.illusion_damage_percent_incoming_melee, -65,
                               keys.illusions_duration)
    if (illusion ~= nil) then
        ...
        illusion:SetHealth(illusion:GetMaxHealth() * Caster:GetHealthPercent() *
                               0.01)
...
function ItemAbility_Good_Lunchbox_Charge(keys)
    ...
    if Caster:HasModifier("modifier_item_better_lunchbox") or
        Caster:HasModifier("modifier_item_best_lunchbox") or
        Caster:HasModifier("modifier_item_god_lunchbox") then return end
    if Caster:GetTeam() ~= Target:GetTeam() and
        Caster:CanEntityBeSeenByMyTeam(Target) then
        local CurrentActiveAbility = Target:GetCurrentActiveAbility()
        if (ItemAbility:IsItem() and CurrentActiveAbility and
            not CurrentActiveAbility:IsItem()) then
            local Charge = ItemAbility:GetCurrentCharges()
...
function ItemAbility_Better_Lunchbox_Charge(keys)
    ...
    local Caster = keys.caster
    local Target = keys.unit
    if Caster:HasModifier("modifier_item_best_lunchbox") or
        Caster:HasModifier("modifier_item_god_lunchbox") then return end
    if Caster:GetTeam() ~= Target:GetTeam() and
        Caster:CanEntityBeSeenByMyTeam(Target) then
        local CurrentActiveAbility = Target:GetCurrentActiveAbility()
...
function ItemAbility_Lunchbox_OnSpellStart(keys)
    ...
    if (ItemAbility:IsItem()) then
        ...
        if (Charge > 0) then
            Caster:Heal(HealAmount, Caster)
            SendOverheadEventMessage(nil, OVERHEAD_ALERT_HEAL, Caster,
                                     HealAmount, nil)
            Caster:GiveMana(HealAmount)
            SendOverheadEventMessage(nil, OVERHEAD_ALERT_MANA_ADD, Caster,
                                     HealAmount, nil)
...
function ItemAbility_HorseKing_OnOpen_SpendMana(keys)
    ...
    do
        if (Caster:GetManaPercent() < keys.SpendManaPercent and
            ItemAbility:GetToggleState()) then
...
function ItemAbility_HorseKing_ToggleOff(keys)
    local ItemAbility = keys.ability
    local Caster = keys.caster
    if (ItemAbility:GetToggleState()) then ItemAbility:ToggleAbility() end
end
...
function ItemAbility_DonationBox_OnSpellStart(keys)
    ...
    if Target:GetUnitName() == "ability_minamitsu_04_ship" or
        Target:GetUnitName() == "ability_margatroid03_doll" then return end
    ...
    SendOverheadEventMessage(nil, OVERHEAD_ALERT_GOLD, Caster, keys.BonusGold,
                             nil)
    Caster:AddExperience(180, DOTA_ModifyXP_CreepKill, false, false)

    local effectIndex = ParticleManager:CreateParticle(
                            "particles/thd2/items/item_donation_box.vpcf",
...
function ItemAbility_DonationGem_BounsGold(keys)
    ...
    if Target:GetTeam() ~= Caster:GetTeam() then
        if (ItemAbility and ItemAbility:IsCooldownReady()) then
            ...
            PlayerResource:SetGold(CasterPlayerID,
                                   PlayerResource:GetUnreliableGold(
                                       CasterPlayerID) + GoldBountyAmount, false)
            ...
            SendOverheadEventMessage(Caster:GetOwner(), OVERHEAD_ALERT_GOLD,
                                     Caster, GoldBountyAmount, nil)
            DebugPrint("ItemAbility_DonationGem_BounsGold| Bounty Gold: " ..
                           tostring(GoldBountyAmount))
        end
    end
end
...
function ItemAbility_PresentBox_RestoreGold(keys)
    ...
    if (ItemAbility:IsItem()) then
        local Caster = keys.caster
        local CasterPlayerID = Caster:GetPlayerOwnerID()
        local RestoreGold = ItemAbility:GetCost() * keys.RestoreGoldPercent *
                                0.01
        PlayerResource:SetGold(CasterPlayerID,
                               PlayerResource:GetUnreliableGold(CasterPlayerID) +
                                   RestoreGold, false)
        Caster:RemoveItem(ItemAbility)
        SendOverheadEventMessage(Caster:GetOwner(), OVERHEAD_ALERT_GOLD, Caster,
                                 RestoreGold, nil)
    end
end
...
function ItemAbility_PresentBox_OnInterval(keys)
    ...
    PlayerResource:SetGold(CasterPlayerID, PlayerResource:GetUnreliableGold(
                               CasterPlayerID) + keys.GiveGoldAmount, false)
...
function ItemAbility_Peach_OnTakeDamage(keys)
    ...
    if (TakeDamageCount == nil) then TakeDamageCount = 0 end

    if (Caster.ItemAbility_Peach_LastTriggerTime == nil or TimeNow -
        Caster.ItemAbility_Peach_LastTriggerTime >= SpeedupDuration) then
        TakeDamageCount = 0
    ...
    if (TakeDamageCount > keys.TakeDamageTrigger) then
        local ModifierCount = round(TakeDamageCount / keys.TakeDamageTrigger) +
                                  1
        ...
        if (Caster:HasModifier(SpeedupModifierName)) then
            if (Caster:GetModifierStackCount(SpeedupModifierName, Caster) ~=
                ModifierCount or
                (ModifierCount == SpeedupMaxModifierStack and TimeNow >
                    Caster.ItemAbility_Peach_LastTriggerTime)) then
                Caster:RemoveModifierByName(SpeedupModifierName)
                ItemAbility:ApplyDataDrivenModifier(Caster, Caster,
                                                    SpeedupModifierName, {
                    duration = SpeedupDuration
                })
                Caster:SetModifierStackCount(SpeedupModifierName, ItemAbility,
                                             ModifierCount)
                TriggerBuff = true
            end
        else
...
function ItemAbility_Anchor_OnTakeDamage(keys)
    ...
    if (Caster.ItemAbility_Anchor_LastTriggerTime == nil or TimeNow -
        Caster.ItemAbility_Anchor_LastTriggerTime >= BuffDuration) then
        TakeDamageCount = 0
    ...
    if (TakeDamageCount > keys.TakeDamageTrigger) then
        ...
        local TriggerBuff = nil
        ModifierCount =
            keys.CritChanceConst + min(ModifierCount, BuffMaxStack) *
                keys.BuffCritChance

        if (Caster:HasModifier(IconModifierName)) then
            if (Caster:GetModifierStackCount(IconModifierName, Caster) ~=
                ModifierCount or
                (ModifierCount == keys.CritChanceConst + BuffMaxStack *
                    keys.BuffCritChance and TimeNow >
                    Caster.ItemAbility_Anchor_LastTriggerTime)) then
                Caster:RemoveModifierByName(IconModifierName)
                ItemAbility:ApplyDataDrivenModifier(Caster, Caster,
                                                    IconModifierName,
                                                    {duration = BuffDuration})
                Caster:SetModifierStackCount(IconModifierName, ItemAbility,
                                             ModifierCount)
                TriggerBuff = true
            end
        else
            ItemAbility:ApplyDataDrivenModifier(Caster, Caster,
                                                IconModifierName,
                                                {duration = BuffDuration})
            Caster:SetModifierStackCount(keys.IconModifierName, ItemAbility,
                                         ModifierCount)
            TriggerBuff = true
        end
...
function ItemAbility_Yuri_OnSpell(keys)
    ...
    if Caster:GetTeam() ~= Target:GetTeam() and
        is_spell_blocked_by_hakurei_amulet(Target) then return end
    -- 敌对
    if Caster:GetTeam() ~= Target:GetTeam() then
        ItemAbility:ApplyDataDrivenModifier(Caster, Caster,
                                            keys.BuffModifierName, {})
        ItemAbility:ApplyDataDrivenModifier(Caster, Target, keys.ModifierName,
                                            {duration = 7})
        ItemAbility:ApplyDataDrivenModifier(Caster, Target, keys.ModifierName,
                                            {duration = 4})
    ...
    Caster:SetThink(function()
        ...
        if (Caster:HasModifier(BuffModifierName) == false or Target:IsAlive() ==
            false or Distance > MaxRange) then
        ...
        if (Distance > FirstDistance) then
            ...
            MoveDistance = (MoveDistance * MoveDistance) / (200 * 200) *
                               MaxSpeed * 0.02
...
function ItemAbility_Yuri_create_line(caster, target)
    local effectIndex = ParticleManager:CreateParticle(
                            "particles/thd2/items/item_lily.vpcf",
                            PATTACH_CUSTOMORIGIN, caster)
    ParticleManager:SetParticleControlEnt(effectIndex, 0, caster, 5,
                                          "attach_hitloc", Vector(0, 0, 0), true)
    ParticleManager:SetParticleControlEnt(effectIndex, 1, target, 5,
                                          "attach_hitloc", Vector(0, 0, 0), true)
    return effectIndex
end
...
function ItemAbility_MoonBow_OnHit(keys)
    ...
    local Caster = keys.caster
    local Target = keys.target
    local damage_to_deal = keys.arrow_damage_const + Caster:GetIntellect() *
                               keys.arrow_damage_int_mult
...
function ItemAbility_InabaIllusionWeapon_Melee_OnAttackLanded(keys)
    ...
    if Caster:IsRealHero() and Caster:GetTeam() ~= Target:GetTeam() then
        ...
        if (ItemCount > 0) then
            ...
            DoCleaveAttack(Caster, Target, ItemAbility, Damage, CleaveRadius,
    ...
    if Caster:GetTeam() ~= Target:GetTeam() then
        ...
        for _, unit in pairs(Targets) do
            if MaxTargets > 0 then
                if unit and unit:IsAlive() and unit ~= Target and
                    Caster:CanEntityBeSeenByMyTeam(unit) then
                    Caster:PerformAttack(unit, true, false, true, false, true,
                                         false, true)
...
function ItemAbility_InabaIllusionWeapon_Cannon_OnAttackLanded(keys)
    ...
    if Caster:IsRealHero() and Caster:GetTeam() ~= Target:GetTeam() then
        ...
        if (ItemCount > 0) then
            local Damage = RandomFloat(CannonDamageMin, CannonDamageMax) *
                               ItemCount
            local Targets = FindUnitsInRadius(Caster:GetTeamNumber(),
            ...
            DebugPrint(
                "ItemAbility_InabaIllusionWeapon_Cannon_OnAttackLanded| Damage:" ..
                    Damage)
        end
    end
end
...
function ItemAbility_HakureiAmulet_OnCreated(keys)
    if keys.ability ~= nil and keys.ability:IsCooldownReady() then
        ...
        keys.caster:AddNewModifier(keys.caster, keys.ability,
...
function ItemAbility_HakureiAmulet_OnDestroy(keys)
    ...
    for i = 0, 5, 1 do -- Search for off-cooldown Linken's Spheres in the player's inventory.
        ...
        if current_item ~= nil then
            if current_item:GetName() == "item_hakurei_amulet" and
                current_item:IsCooldownReady() then
...
function ItemAbility_HakureiAmulet_OnIntervalThink(keys)
    if not keys.caster:HasModifier("modifier_item_sphere_target") then
        if keys.caster:HasModifier("modifier_item_hakurei_amulet_icon") then -- blocked spell
            for i = 0, 5, 1 do
                local current_item = keys.caster:GetItemInSlot(i)
                if current_item ~= nil and current_item:GetName() ==
                    "item_hakurei_amulet" then
                    -- current_item:StartCooldown(current_item:GetCooldown(current_item:GetLevel()))
                    current_item:StartCooldown(
                        GetCurrentCoolDown(current_item, keys.caster))
                end
            end
            keys.caster:RemoveModifierByName("modifier_item_hakurei_amulet_icon")
        else -- reset modifier
...
function ItemAbility_Qijizhixing_OnSpellStart(keys)
    ...
    SendOverheadEventMessage(nil, OVERHEAD_ALERT_HEAL, target,
                             keys.HealAmount + caster:GetMaxMana() * 0.25, nil)
...
function ItemAbility_Ganggenier_OnDealDamage(keys)
    ...
    if (caster.ganggenierlock == nil) then caster.ganggenierlock = false end

    if (caster.ganggenierlock == true) then return end
...
function ItemAbility_Morenjingjuan_Antiblink_OnSpellStart(keys)
    ...
    local attackspeed = keys.AttackSpeedIncrese *
                            (caster:GetMaxMana() - target:GetMaxMana())
    ...
    caster:SetModifierStackCount("modifier_item_morenjingjuan_buff", ability,
                                 attackspeedint)
    ...
    local effectIndex2 = ParticleManager:CreateParticle(
                             "particles/thd2/items/item_morenjingjuan.vpcf",
                             PATTACH_CUSTOMORIGIN, caster)
    ParticleManager:SetParticleControlEnt(effectIndex2, 0, caster, 5,
                                          "follow_origin", Vector(0, 0, 0), true)
    ...
    GameRules:GetGameModeEntity():SetContextThink(
        DoUniqueString('item_ability_morenjingjuan'), function()
            if GameRules:IsGamePaused() then return 0.03 end
            count = count + 0.03

            if target:HasModifier("modifier_item_morenjingjuan_antiblink") ==
                false then
                ParticleManager:DestroyParticleSystem(effectIndex, true)
                return nil
            end

            if target.is_Iku_02_knock == true then return 0.03 end
...
function ItemAbility_phoenix_wing_burn(keys)
    ...
    local target = keys.target
    local ability = keys.ability
    local distance = GetDistanceBetweenTwoVec2D(caster:GetOrigin(),
                                                target:GetOrigin())
    local dealdamage =
        keys.damage + caster:GetMaxHealth() * keys.damage_percent / 100
...
function OnZunGlasses_Take_Damage(keys)
    ...
    if keys.ability:IsCooldownReady() and caster:IsRealHero() then
        if caster:GetHealth() == 0 and caster:GetClassname() ~=
            "npc_dota_hero_necrolyte" and
            ...
            keys.ability:StartCooldown(GetCurrentCoolDown(keys.ability,
                                                          keys.caster))
        end
    end
    -- 原代码
...
function OnTentacle_Effect(keys)
    ...
    if Caster and Target and Caster:GetTeam() ~= Target:GetTeam() then
        if (not keys.ApplyToTower or keys.ApplyToTower == 0) and
            Target:IsBuilding() then return end
        local randomInt = RandomInt(0, 100)
