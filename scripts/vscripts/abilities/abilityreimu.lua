...
REIMU01_GRAVITY = 500
REIMU01_INTERVAL = 0.01
REIMU01_START_HEIGHT = 680
REIMU01_BOUNCE_HEIGHT = 80
REIMU01_BOUNCE_FACTOR = 0.8
REIMU01_BOUNCE_FACTOR_FOLLOW = 0.9
    ...
    ballunit:SetContextThink("OnReimu01Release", function()
        ...
        if uz <= ballunit.tReimu01Elements.OriginZ + REIMU01_BOUNCE_HEIGHT then
            ...
            if ballunit.tReimu01Elements.Ball.t > 0 then
                damageFactor = keys.DamageFollowPercentage / 100
                stunDurationReal = keys.StunDurationFollow
                speedFactor = REIMU01_BOUNCE_FACTOR_FOLLOW
            end

            ballunit.tReimu01Elements.Ball.v =
                ballunit.tReimu01Elements.Ball.v * -speedFactor
            vec = Vector(headOrigin.x, headOrigin.y, ballunit.tReimu01Elements
                             .OriginZ + REIMU01_BOUNCE_HEIGHT + 0.1)
            ballunit.tReimu01Elements.ballVector = vec
            ballunit.tReimu01Elements.Ball.t =
                ballunit.tReimu01Elements.Ball.t + 1
            ...
            local DamageTargets =
                ...
                DOTA_UNIT_TARGET_TEAM_ENEMY,
                                  ...
                                  FIND_CLOSEST, false)
            local damageFinal = (keys.FirstDamage +
                                    FindTelentValue(caster,
...
function AbilityReimu:OnReimu03Start(keys)
    ...
    if (caster:GetTeam() == keys.target:GetTeam()) then
        keys.ability:ApplyDataDrivenModifier(caster, keys.target,
        ...
        GameRules:GetGameModeEntity():SetContextThink(
            DoUniqueString('Release Effect'), function()
                if GameRules:IsGamePaused() then return 0.03 end
                keys.target:RemoveModifierByName(
