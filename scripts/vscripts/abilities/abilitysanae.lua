		...
		function(i)
	    	...
	    	for _,v in pairs(targets) do
				local damage_table = {
					...
					victim = v,
					attacker = caster,
					damage = keys.ability:GetAbilityDamage()+FindTelentValue(caster,"special_bonus_unique_lion_2"),
...
function OnSanae02SpellStart(keys)
	...
	for _,v in pairs(targets) do
		local damage_table = {
			...
			victim = v,
			attacker = caster,
			damage = keys.ability:GetAbilityDamage()+FindTelentValue(caster,"special_bonus_unique_lion_4"),
			damage_type = keys.ability:GetAbilityDamageType(), 
			damage_flags = keys.ability:GetAbilityTargetFlags()
		}
		UnitDamageTarget(damage_table)
