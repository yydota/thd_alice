...
function OnReisenOldExSpellSuccess(keys)
	...
	local deal_damage = caster:GetPrimaryStatValue()*0.3
	...
	elseif RandomNumber <= deal_chance and caster:HasModifier("ability_reisenold04_modifier")==false then		
		UnitDamageTarget(damage_table)
		keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_reisenexold_slow", {})
		StartSoundEvent("Voice_Thdots_Reisen.AbilityReisen01", caster)
	end	
...
function OnReisenOld03SpellStart( keys )
	local caster = keys.caster
	ReisenOld03effectIndex = ParticleManager:CreateParticle("particles/heroes/reisen/reisenold03_channel.vpcf", PATTACH_CUSTOMORIGIN, caster)
	ParticleManager:SetParticleControlEnt(ReisenOld03effectIndex, 0, caster, 5, "attach_attack2", Vector(0,0,0), true)
end

function OnReisenOld03SpellEnd01( keys )
	local caster = keys.caster
	ParticleManager:DestroyParticleSystem(ReisenOld03effectIndex,false)
end

function OnReisenOld03SpellEnd02( keys )
	local caster = keys.caster
	ParticleManager:DestroyParticleSystem(ReisenOld03effectIndex,true)
end
...
function OnReisenOld03SpellHit(keys)
	...
	local deal_damage=keys.ability:GetAbilityDamage()+Damage*keys.DamageMulti
	local damage_table = {
				ability = keys.ability,
			    victim = keys.target,
			    attacker = caster,
