...
function Komachi01_Think(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	local modifier = caster:FindModifierByName("modifier_thdots_komachi_blink")
	if modifier:GetStackCount()<keys.stackCount and caster:HasModifier("modifier_thdots_komachi_time_to_add_blink")==false then
	...
	if modifier~=nil then
		...
		if modifier:GetStackCount()>0 then
			...
			if modifierThink~=nil then
				...
				keys.ability:StartCooldown(10 - FindTelentValue(caster,"special_bonus_unique_elder_titan"))
			end
		end
	end
end
...
function Komachi01_AddBlink(keys)
	...
	if modifier:GetStackCount() < keys.stackCount then
		modifier:IncrementStackCount()
	end
end
	...
	if ability~=nil then
		...
		if modifier == nil then
			...
			if modifier:GetStackCount() < 9 then
				modifier:IncrementStackCount()
				modifier:SetDuration(10.0,true)
			end
		end
