...
function OnPatchouliAddFire(keys)
	local caster=keys.caster
	if ElementsNum==nil then ElementsNum=1 end
	if ElementsNum==1 and ElementsName[1]~=nil then
		caster:RemoveModifierByName(ElementsName[1])	
	elseif ElementsNum==2 and ElementsName[2]~=nil then		
		caster:RemoveModifierByName(ElementsName[2])	
	elseif ElementsNum==3 then		
		ElementsNum=1
		caster:RemoveModifierByName(ElementsName[1])	
	end
	ElementsName[ElementsNum]="modifier_thdots_patchouli_fire"
...
function OnPatchouliAddWater(keys)
	...
	ElementsName[ElementsNum]="modifier_thdots_patchouli_water"
	ElementsNum=ElementsNum+1
end
...
function OnPatchouliAddWood(keys)
	...
	ElementsName[ElementsNum]="modifier_thdots_patchouli_wood"
	ElementsNum=ElementsNum+1
end
...
function OnPatchouliAddMetal(keys)
	...
	ElementsName[ElementsNum]="modifier_thdots_patchouli_metal"
	ElementsNum=ElementsNum+1
end
...
function OnPatchouliAddEarth(keys)
	...
	ElementsName[ElementsNum]="modifier_thdots_patchouli_earth"
	ElementsNum=ElementsNum+1
end
...
function OnPatchouliInvoke(keys)
	...
	if SwapAbilityName == nil then SwapAbilityName="invoker_empty1" end
...
function OnPatchouliFireWood(keys)
	...
	if GridNav:IsNearbyTree(keys.target_points[1], keys.Radius, false) then
		tree = 1
	end
...
function OnPatchouliFireEarth(keys)
	...
	local deal_duration = keys.Duration + firelevel*keys.FireMultiple
...
function OnPatchouliXianzhezhishi(keys)
	...
	if FindTelentValue(caster,"special_bonus_unique_invoker_6") == 1 then
		ability_xianzhezhishi:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_patchouli_xianzhezhishi_add", {Duration = 5.5})
	else
		ability_xianzhezhishi:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_patchouli_xianzhezhishi_add", {Duration = 4.0})
	end
