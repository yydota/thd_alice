...
function CirnoFindIceboundsInRadius(pos, radius)
    ...
    table.sort(icebounds, function(a, b)
...
function CirnoCreateIcebound(caster, pos, lifetime)
    ...
    local effect_index = ParticleManager:CreateParticle(
                             "particles/units/heroes/hero_invoker/invoker_ice_wall_snow_ground.vpcf",
    ...
    if not game_ent.started_cirno_gc then
        ...
        game_ent:SetContextThink("cirno_ability_gc", function()
...
function CirnoBreakIcebound(icebound, OnBreak, count)
    ...
    local icebounds = CirnoFindIceboundsInRadius(icebound.pos,
                                                 ABILITY_CIRNO_ICEBOUND_HITBOX)
    ...
    for _, ib in pairs(icebounds) do
        ...
        Timer.Loop(tostring(ib) .. "ready_to_break")(
            ...
            function() CirnoBreakIcebound(ib, OnBreak, count + 1) end)
    end
...
function CirnoBreakIceboundsInRadius(caster, pos, radius)
    ...
    if AbilityCirno02 then
        cirno02_StunEnemyDuration = AbilityCirno02:GetLevelSpecialValueFor(
                                        "stun_enemy_duration",
                                        AbilityCirno02:GetLevel())
    ...
    if AbilityCirno04 then
        cirno04_freeze_damage_table = {
            victim = nil,
            attacker = caster,
            damage = AbilityCirno04:GetLevelSpecialValueFor("freeze_damage",
                                                            AbilityCirno04:GetLevel()),
            damage_type = AbilityCirno04:GetAbilityDamageType()
        }
    end

    local icebounds = CirnoFindIceboundsInRadius(caster:GetAbsOrigin(),
                                                 ABILITY_CIRNO_ICEBOUND_HITBOX)
    ...
    for _, ib in pairs(icebounds) do
        if not ib.ready_to_break then
            ...
            CirnoBreakIcebound(ib, function(icebound, count, nearby_icebounds)
                ...
                for _, v in pairs(enemies) do
                    if not v:IsMagicImmune() or
                        v:HasModifier("modifier_thdots_crino04_freeze") then
                        if AbilityCirno04 and
                            v:HasModifier("modifier_thdots_crino04_freeze") then
                            v:RemoveModifierByName(
                            ...
                            ParticleManager:DestroyParticleSystem(
                                v.ability_cirno_04_effect_index, true)
                        ...
                        if AbilityCirno02 then
                            ...
                            if is_stun_unit then
                                UtilStun:UnitStunTarget(caster, v,
                                                        cirno02_StunEnemyDuration)
                            end
                        end
                    end
                end
                if caster.ability_cirno_04_effect_index_table ~= nil then
                    for k, v in
                        pairs(caster.ability_cirno_04_effect_index_table) do
                ...
                local effectIndex = ParticleManager:CreateParticle(
                                        "particles/heroes/cirno/ability_cirno_02.vpcf",
...
function OnCirno01SpellStart(keys)
    ...
    local Targets = FindUnitsInRadius(Caster:GetTeamNumber(),
                                      Caster:GetAbsOrigin(), nil, keys.radius,
...
function OnCirno02SpellStart(keys)
    if (CirnoBreakIceboundsInRadius(keys.caster, keys.caster:GetOrigin(),
                                    keys.radius) == 0) then
        UtilStun:UnitStunTarget(keys.caster, keys.caster,
                                keys.stun_self_duration)
    end
end
...
function OnCirno03SpellStart(keys)
    ...
    local HasDamaged = {}
    Caster:SetContextThink("cirno03_in_spelling", function()
        if GameRules:IsGamePaused() then return 0.03 end
        local VecPos = VecStart + Direction * MovePerTick * tick
        local enemies = FindUnitsInRadius(Caster:GetTeamNumber(), VecPos, nil,
                                          keys.width,
                                          Ability:GetAbilityTargetTeam(),
                                          Ability:GetAbilityTargetType(),
                                          Ability:GetAbilityTargetFlags(),
...
function OnCirno04SpellStart(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle(
                            "particles/heroes/cirno/ability_cirno_04.vpcf",
                            PATTACH_CUSTOMORIGIN, nil)
    ParticleManager:SetParticleControlEnt(effectIndex, 0, Caster, 5,
                                          "follow_origin", Vector(0, 0, 0), true)
    ParticleManager:SetParticleControlEnt(effectIndex, 3, Caster, 5,
                                          "follow_origin", Vector(0, 0, 0), true)
    ...
    Caster:SetContextThink("cirno04_in_spelling", function()
        ...
        local enemies = FindUnitsInRadius(Caster:GetTeamNumber(),
                                          Caster:GetOrigin(), nil, keys.radius,
                                          Ability:GetAbilityTargetTeam(),
                                          Ability:GetAbilityTargetType(),
                                          Ability:GetAbilityTargetFlags(),
                                          FIND_ANY_ORDER, --[[FIND_CLOSEST,]]
        ...
        if tick >= tick_max or not Caster:IsAlive() then
            ...
            ParticleManager:DestroyParticleSystem(
                Caster.ability_cirno_04_caster_effect_index, true)
            return nil
        end
        return TickInterval
    end, 0)
...
function OnCrino04IconDestroy(keys)
    local caster = EntIndexToHScript(keys.caster_entindex)
    keys.ability:ApplyDataDrivenModifier(caster, caster,
                                         "modifier_cirno04_bonus_action", nil)
