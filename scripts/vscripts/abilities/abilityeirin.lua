if AbilityEirin == nil then AbilityEirin = class({}) end

function OnEirin02SpellStart(keys)
    local caster = EntIndexToHScript(keys.caster_entindex)
    local vecCaster = caster:GetOrigin()
    THDReduceCooldown(keys.ability,
                      FindTelentValue(caster, "special_bonus_unique_silencer"))
...
function OnEirin02SpellHit(keys)
    ...
    if (target:GetTeam() ~= caster:GetTeam()) then
        ...
        local spellPoint = Vector(keys.ability:GetContext(
                                      "ability_eirin02_spell_x"),
                                  keys.ability:GetContext(
                                      "ability_eirin02_spell_y"),
                                  keys.ability:GetContext(
                                      "ability_eirin02_spell_z"))
        local dis = GetDistanceBetweenTwoVec2D(spellPoint, target:GetOrigin())
        local duration = (dis - dis % 150) / 150 * keys.StunDuration
        if (duration >= 2.0) then duration = 2.0 end
    ...
    Timer.Loop 'ability_eirin02_spell_heal'(0.2, 20, function(i)
        local targets = FindUnitsInRadius(caster:GetTeam(), -- caster team
        vecTarget, -- find position
        nil, -- find entity
        keys.Radius, -- find radius
        DOTA_UNIT_TARGET_TEAM_BOTH, keys.ability:GetAbilityTargetType(), 0,
                                          FIND_CLOSEST, false)
        if (GetDistanceBetweenTwoVec2D(vecTarget, caster:GetOrigin()) <=
            keys.Radius) then caster:Heal(keys.Damage / 5, caster) end
        for k, v in pairs(targets) do
            if (v:GetTeam() ~= caster:GetTeam()) then
                ...
                ParticleManager:SetParticleControl(healEffectIndex, 0,
                                                   v:GetOrigin())
                Timer.Wait 'ability_eirin02_remove_heal_effect'(1, function()
                    ParticleManager:DestroyParticleSystem(healEffectIndex, true)
                end)
            end
        end
    end)
    Timer.Wait 'ability_eirin02_remove_effect'(4, function()
...
function OnEirin03EffectInvisible(keys)
    local caster = EntIndexToHScript(keys.caster_entindex)
    local target = keys.target
    target:AddNewModifier(nil, nil, "modifier_invisible",
                          {duration = keys.Duration})
end

function OnEirin04SpellStart(keys)
    local caster = EntIndexToHScript(keys.caster_entindex)
    THDReduceCooldown(keys.ability, FindTelentValue(caster,
                                                    "special_bonus_unique_silencer_4"))
    keys.ability.target = keys.target
    local effectIndex = ParticleManager:CreateParticle(
                            "particles/heroes/eirin/ability_eirin_04.vpcf",
                            PATTACH_CUSTOMORIGIN, keys.target)
    ParticleManager:SetParticleControlEnt(effectIndex, 0, keys.target, 5,
