...
G_Bots_Ability_Add = {
	...
	{1,2,1,2,1,  6,1,2,2,11,  3,6,3,3,12, 3,0,6,0,15,  0,0,0,0,17  }, -- not using below
	{1,2,1,2,1,  6,1,2,2,10,  3,6,3,3,13, 3,0,6,0,14,  0,0,0,0,17  },
	{1,2,3,2,2,  6,2,3,3,11,  3,6,1,1,13, 1,0,6,0,15,  0,0,0,0,16  }
	...
	elseif text == "-random" then
		if GameRules:State_Get() > 2 then -- after shuffle
			if PlayerResource:HasSelectedHero(plyid) == false then
				...
				print("player " .. PlayerResource:GetPlayerName(plyid) .. " trys to cheat!") --有人尝试刷钱作弊
			end
		end
  ...
  if hero:IsHero() then
		...
		if plyid > 0 then
			...
			if Hero_Cloth[hero:GetClassname()] ~= nil then
				local cloth = Hero_Cloth[hero:GetClassname()][ string.format("cloth0%d",cloth_id) ]
				if cloth ~= nil then
					hero:SetModel(cloth)
...
function THDOTSGameMode:PrecacheHeroResource(hero)
	...
	elseif(heroName == "npc_dota_hero_invoker")then		
		...
		ability:SetLevel(1)
		ability:ApplyDataDrivenModifier(hero,hero,"modifier_thdots_patchouli_select",{})	
...
function THDOTSGameMode:OnGameRulesStateChange(keys)
	...
	elseif newState == DOTA_GAMERULES_STATE_STRATEGY_TIME then
		...
		for i =0,63 do
			if PlayerResource:GetPlayer(i) ~= nil then
				if PlayerResource:HasSelectedHero(i) == false then
					SendToServerConsole('dota_pause')
					HostSay(".检测到有玩家未选择人物, 游戏已自动暂停")
					HostSay(".当所有人准备好后请输入 -pause 解除暂停")
					HostSay(".如果进入游戏后仍然未选择, 游戏会自动为玩家随机一个人物, 但没有额外赏金")
					break --防止多次暂停
				else
					--对帕秋莉玩家的提醒
					if PlayerResource:GetSelectedHeroName(i) == "npc_dota_hero_invoker" then
						HostSay(".由于快捷键数量限制，帕秋莉必须5选4元素，两两合成10种魔法")	
						HostSay(".帕秋莉开局使用的第一个元素将会被舍弃,并留下空位当做合成魔法的键位")	
						HostSay(".帕秋莉在25级时会获得7个技能点，用于加满所有天赋和技能")
					end
				end
			end
		end
	...
	elseif newState == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then  --游戏到达 00:00(第一次刷符)
		...
		G_Rune_PowerUp_Spwner_List = Entities:FindAllByClassname("dota_item_rune_spawner_powerup")
		GameRules:GetGameModeEntity():SetContextThink(
			"Rune_Bounty_Remover", --check if the old -5min bounty picked
			function()
				for _,v in pairs(G_Rune_Bounty_Spwner_List) do
					if G_Rune_Bounty_List[_] == 0 then  --should delete
						local nearbyRunes = Entities:FindAllByClassnameWithin("dota_item_rune", v:GetOrigin(), 100)
		...
		GameRules:GetGameModeEntity():SetContextThink(
			"Rune_Bounty_PreChecker",  --if not picked now, it's not picked ,not the new one
			function()
				for _,v in pairs(G_Rune_Bounty_Spwner_List) do
					local nearbyRunes = Entities:FindAllByClassnameWithin("dota_item_rune", v:GetOrigin(), 100)
					local is_empty = true
